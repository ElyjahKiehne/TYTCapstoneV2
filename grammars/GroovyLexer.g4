...
@members {
    ...
    private Stack<Brace> braceStack = new Stack<Brace>();
    private Brace topBrace = default(Brace);
    ...
    public override void emit(IToken token)
    {
     ...
     // Hook to be overridden by GroovyLangLexer
    protected virtual void RollbackOneChar() { }

    private void pushBrace(Brace b)
    {
        braceStack.Push(b);
        topBrace = braceStack.Peek();
    }

    private void popBrace()
    {
        braceStack.Pop();
        topBrace = braceStack.Count > 0 ? braceStack.Peek() : (Brace?)null;
    }

    private bool isSlashyStringAllowed()
    {
        bool isLastTokenOp = ALLOWED_OP_SET.Contains(lastTokenType);
        bool res = isLastTokenOp || tlePos == tokenIndex;
        return res;
    }
